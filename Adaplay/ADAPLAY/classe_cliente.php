...
Class Cliente{
    ...
    {   $res = array();
        $cmd = $this->pdo->query("SELECT * FROM mensagem_clients ORDER BY id desc");
    ...
    {
        $cmd = $this->pdo->prepare("INSERT INTO mensagem_clients  (nome, email, assunto, telefone, mensagem) VALUES (:n, :e, :a, :t, :m)");
    ...
    {
        $cmd = $this->pdo->prepare("DELETE FROM mensagem_clients  WHERE id = :id");
        $cmd->bindValue(":id", $id);
        $cmd->execute();
    }
    ...
    {
        $res = array();
        $cmd = $this->pdo->prepare("SELECT * FROM mensagem_clients  WHERE id = :id");
        $cmd->bindValue(":id",$id);
        $cmd->execute();
    ...
    {
        $cmd = $this->pdo->prepare("UPDATE mensagem_clients  SET nome = :n, email= :e, assunto= :a, telefone = :t, mensagem= :m WHERE id = :id");
        $cmd->bindValue(":n", $nome);
        $cmd->bindValue(":e", $email);
        $cmd->bindValue(":a", $assunto);
        $cmd->bindValue(":t", $telefone);
